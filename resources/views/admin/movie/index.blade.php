        ...
        <script>
            ...
            $(function () {
                $('#moviesTable').DataTable({
                    processing: true,
                    // data untuk database diproses secara serverside (controller)
                    serverSide: true,
                    // routing menuju fungsi yang memproses data untuk database
                    ajax: "{{ route('admin.movies.datatables') }}",
                    // urutkan column (td), pastikan urutan sesuai th
                    // data: 'nama' -> nama di ambil dari rawColumns jika addColumns, atau field dari model fillable
                    columns: [
                        { data : 'DT_RowIndex', name: 'DT_RowIndex', orderable:false, searchable:false},
                        { data : 'poster_img', name: 'poster_img', orderable:false, searchable:false},
                        { data : 'title', name: 'title', orderable:true, searchable:true},
                        { data : 'activated_badge', name: 'activated_badge', orderable:false, searchable:false},
            ...
            function showModal(item) {
                ...
                let content = `
                    <img src="${image}" width="120" class="d-block mx-auto my-3" onerror="this.style.display='none'">
                    <ul>
