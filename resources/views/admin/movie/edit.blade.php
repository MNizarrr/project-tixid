...
@section('content')
    <div class="w-75 d-block mx-auto my-5">
        <form action=" {{ route('admin.movies.update', $movie->id)}} " method="POST" enctype="multipart/form-data">
            {{-- enctype="multipart/form-data" : mengambil file yang di upload secara utuh bukan hanya nama file nya --}}
            @csrf
            @method('PUT')
            <div class="row mb-3">
                <div class="col-6">
            ...
            <div class="row mb-3">
                <div class="col-6">
                    ...
                    <input type="number" name="age_rating" id="age_rating" class="form-control @error('age_rating')
                    is-invalid @enderror" value="{{ $movie->age_rating }}">
                    @error('age_rating')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="col-6">
                    ...
                    <input type="file" name="poster" id="poster" class="form-control @error('poster')
                    is-invalid @enderror" value="{{ $movie->poster }}">
                    @error('poster')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="description" id="description" rows="5" class="form-control
                            @error('description') is-invalid @enderror" value="{{ $movie->description }}">{{ $movie->description }}</textarea>
                @error('description')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
